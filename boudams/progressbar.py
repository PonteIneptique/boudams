# This progress bar is heavily inspired by https://github.com/mittagessen/kraken/blob/master/kraken/lib/progress.py
...
class EarlyStoppingColumn(ProgressColumn):
    ...
    def render(self, task) -> Text:
        ...
        text = f'early_stopping: ' \
               ...
               f'best: {self._trainer.early_stopping_callback.best_score:.4f}'
        return Text(text, justify="left")
...
class LrColumn(ProgressColumn):
    def __init__(self, trainer):
        super(LrColumn, self).__init__()
        self._trainer = trainer

    @staticmethod
    def get_lr(trainer):
        for p in trainer.optimizers[0].param_groups:
            return p["lr"]

    def render(self, task: "Task") -> RenderableType:
        text = f'lr: {self.get_lr(self._trainer):.5f}'
        return Text(text, justify="left")


class BoudamsProgressBar(ProgressBarBase):
    def __init__(self,
                 refresh_rate: int = 1,
                 leave: bool = True,
